    ...
    await prisma.tracksInPlaylists
      ...
      .catch((e) => {
        if (e.code === "P2002") {
          return res
            .status(400)
            .json({ error: "Track already in the playlist" });
        } else if (e instanceof Prisma.PrismaClientKnownRequestError) {
          console.log(`Prisma Error ${e.code}: ${e.message}`);
  ...
  async (req, res) => {
    ...
    if (!trackId) {
      return res.status(404).json({
        error: "trackId not specified",
      });
    }
    // ========================================

    const playlistId = await prisma.playlist
      .findUnique({
        where: { userId_slug: { userId: address, slug } },
      })
      ...
      .catch((e) => {
        if (e.code === "P2025") {
          ...
          console.log(`Prisma Error ${e.code}: ${e.message}`);
          res.status(400).json({ error: "Playlist updating error" });
        } else {
          console.log(e);
    ...
    await prisma.tracksInPlaylists
      .delete({
        where: { trackId_playlistId: { trackId, playlistId } },
      })
      .then(() => {
        return res.status(200).json({ ok: true });
      })
      .catch((e) => {
        if (e.code === "P2025") {
          return res
            .status(404)
            .json({ error: "Track is not in the playlist" });
        } else if (e instanceof Prisma.PrismaClientKnownRequestError) {
          console.log(`Prisma Error ${e.code}: ${e.message}`);
          return res.status(400).json({ error: "Playlist updating error" });
        } else {
